...
export class AlumnosService {
  ...
  public registrarAlumno(data: any): Observable<any> {
    return this.http.post<any>(`${environment.url_api}/api/alumno/registro/`, data, httpOptions);
  }
  ...
  public obtenerAlumnoPorId(id: number): Observable<any> {
    const token = this.getSessionToken();
    const headers = new HttpHeaders({ 'Authorization': 'Bearer ' + token });
    return this.http.get<any>(`${environment.url_api}/alumno-edit/${id}/`, { headers: headers });
  }

  public actualizarAlumno(id: number, data: any): Observable<any> {
    const token = this.getSessionToken();
    const headers = new HttpHeaders({ 'Authorization': 'Bearer ' + token, 'Content-Type': 'application/json' });
    return this.http.put<any>(`${environment.url_api}/alumno-edit/${id}/`, data, { headers: headers });
  }

  public eliminarAlumno(id: number): Observable<any> {
    const token = this.getSessionToken();
    const headers = new HttpHeaders({ 'Authorization': 'Bearer ' + token });
    return this.http.delete<any>(`${environment.url_api}/alumno-edit/${id}/`, { headers: headers });
  }
}
